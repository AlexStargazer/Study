...
def build_state_block(gs: GameState) -> str:
    ...
    # Сигнал тревоги при критическом ресурсе — без упоминания слова «СРЕДНИЙ»/«НИЗКИЙ» в явном виде
...

{
    ...
    "intro": (
        ...
        "и не связан ли он с произошедшим.\n"
        "Перед вами Виктор — дежурил в ту смену, нервничает, даёт показания. "
        "Он может знать больше, чем говорит. Или не знать ничего важного. "
...
— Давление 1: начинаешь путаться в деталях, называешь разное время («около девяти»,
  потом «может, полдесятого»).
— Давление 2: под нажимом упоминаешь что-то лишнее — «ну, я ненадолго отлучался»
  ИЛИ «там был какой-то фургон, но я не смотрел» ИЛИ «Серёга тоже был поблизости».
  Выбери один из трёх вариантов — тот, который кажется наименее опасным для тебя.
  Добавь [SLIPPED] в скрытой строке.
— Давление 3: признаёшься в отлучке или в том, что знал про Серёгу. Добавь [CONFESSED].

КЛЮЧЕВОЕ: давление не раскрывает правду полностью — оно даёт неопределённые
фрагменты. Детектив не может знать, признание это или защитная выдумка.
Виктор сам не уверен, что важно, а что нет.
