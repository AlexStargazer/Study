    ...
    def _test_sheets_on_startup():
        ...
        try:
            ...
            if sheet:
                ...
                if not existing:
                    header = [["timestamp", "role", "content",
                               "scenario", "turn", "time_seconds", "participant_id"]]
                    sheet.append_rows(header, value_input_option="RAW", table_range="A1")
...
ЗАДЕРЖКА И ЗАШУМЛЕНИЕ ОБРАТНОЙ СВЯЗИ:
Никогда не описывай результат действия участника немедленно и однозначно.
Вместо этого — только начало процесса: «бригада выдвинулась», «запрос отправлен»,
«разговор начался». Результат появляется позже, как фоновое обновление ситуации,
не как прямой ответ на действие.

РАЗРЫВ ПРИЧИННЫХ СВЯЗЕЙ:
Ситуация развивается частично независимо от действий участника.
Иногда она улучшается или ухудшается по внешним причинам, не связанным с последним
действием — погода, другие люди, случайные факторы. Не строй нарратив где каждое
действие участника имеет очевидный и немедленный эффект.
Участник не должен легко понять, помогло ли его решение или нет.
...

{
    ...
    "opening": (
        "Команда собралась на первую рабочую встречу. "
        "Советники уже прислали свои рекомендации — они расходятся. "
        "В почте несколько непрочитанных писем. "
        "Заказчик ждёт первого отчёта к концу недели."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 35.0 | Сценарий: Управление проектом
...

{
    ...
    "opening": (
        "Робот на поверхности. Все системы в норме. "
        "На экране — первые данные сканирования: местность ровная, "
        "но в одном направлении приборы фиксируют слабую аномалию. "
        "База на связи."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 35.0 | Сценарий: Межпланетная миссия
...

{
    ...
    "opening": (
        "Поступают первые сводки. Вода прибывает. "
        "Часть данных противоречит друг другу — непонятно, "
        "насколько ситуация серьёзна в дальних районах. "
        "Бригады на связи, ждут команды."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 35.0 | Сценарий: Кризисное управление
...

{
    ...
    "opening": (
        "Виктор садится напротив. "
        "Кладёт пропуск на стол, смотрит в сторону."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 35.0 | Сценарий: Разговор с информантом
...

{
    ...
    "opening": (
        "Утренний брифинг. "
        "Данные за последние сутки на экране — картина неоднозначная. "
        "Команда в ожидании. "
        "Один из специалистов уже высказался, другой с ним не согласен."
    ),
    "resource_start": 1,
    "opening": (
        "Утренний брифинг. Данные за последние сутки на экране — картина "
        "неоднозначная. Команда в ожидании. Один из специалистов уже высказался, "
        "другой с ним не согласен."
    ),
    "prompt": """\
VERSION: 35.0 | Сценарий: Борьба с вирусом
...

{
    ...
    "opening": (
        "Вы открываете записи. Магазин работает в обычном режиме. "
        "Несколько эпизодов за последние часы выглядят немного необычно — или нет. "
        "Трудно сказать."
    ),
    "resource_start": 0,
    "opening": (
        "Вы открываете записи. Магазин работает в обычном режиме. Несколько "
        "эпизодов за последние часы выглядят немного необычно — или нет. "
        "Трудно сказать."
    ),
    "prompt": """\
VERSION: 35.0 | Сценарий: Видеонаблюдение
...
ГЛАВНОЕ ПРАВИЛО — СОХРАНЕНИЕ НЕОПРЕДЕЛЁННОСТИ:
Никогда не раскрывай однозначно — происходит что-то плохое или нет.
Все эпизоды на записях допускают разные интерпретации: может быть нарушение,
может быть норма. Если участник звонит другу — друг воспринимает информацию
спокойно или обеспокоенно, но без однозначного вывода.
Ситуация остаётся неоднозначной до конца сессии.
...

{
    ...
    "opening": (
        "Обычное утро в офисе. Коллега на месте, работает."
    ),
    "resource_start": 0,
    "opening": (
        "Обычное утро в офисе. Коллега на месте, работает."
    ...
    "prompt": """\
VERSION: 35.0 | Сценарий: Поведение коллеги
...
ГЛАВНОЕ ПРАВИЛО — СОХРАНЕНИЕ НЕОПРЕДЕЛЁННОСТИ:
Причины изменений в поведении коллеги не раскрываются никогда — ни прямо, ни намёком.
Если участник спрашивает коллегу напрямую — коллега отвечает уклончиво, расплывчато
или переводит тему. Никакого «я устал», «у меня проблемы дома», «всё нормально» —
любой ответ оставляет больше вопросов чем даёт ответов.
Ситуация может немного меняться, но её суть остаётся неясной до конца сессии.
...

{
    ...
    "opening": (
        "Очередной день наблюдений. В протоколе — плановые процедуры. "
        "Один из показателей ведёт себя немного иначе, чем обычно. "
        "Команда не комментирует."
    ),
    "resource_start": 0,
    "opening": (
        "Очередной день наблюдений. В протоколе — плановые процедуры. Один из "
        "показателей ведёт себя немного иначе, чем обычно. Команда не комментирует."
    ),
    "prompt": """\
VERSION: 35.0 | Сценарий: Клинические испытания
...
ГЛАВНОЕ ПРАВИЛО — СОХРАНЕНИЕ НЕОПРЕДЕЛЁННОСТИ:
Никогда не давай участнику однозначного ответа — является ли отклонение проблемой.
Если участник спрашивает команду или координатора — они отвечают нейтрально,
ссылаются на протокол, говорят что «наблюдают». Никаких «это серьёзно» или
«всё в порядке». Данные остаются противоречивыми до конца сессии.
...

{
    ...
    "opening": (
        "Вы подъезжаете к участку. Бригада работает. "
        "На первый взгляд всё как обычно."
    ),
    "resource_start": 0,
    "opening": (
        "Вы подъезжаете к участку. Бригада работает. На первый взгляд всё как обычно."
    ),
    "prompt": """\
VERSION: 35.0 | Сценарий: Стройка
...
ГЛАВНОЕ ПРАВИЛО — СОХРАНЕНИЕ НЕОПРЕДЕЛЁННОСТИ:
Никогда не раскрывай однозначно — нарушается ли что-то или нет.
Прораб говорит что всё нормально, но его слова не дают уверенности.
Знакомый по телефону реагирует неоднозначно. Нет эксперта который скажет правду.
Ситуация остаётся в зоне «может быть проблема, может быть норма» до конца сессии.
...

{
    ...
    "opening": (
        "Работы продолжаются. Во дворе шумно. "
        "Вы смотрите из окна — что-то изменилось со вчера, "
        "но точно не можете сказать, что именно."
    ),
    "resource_start": 0,
    "opening": (
        "Работы продолжаются. Во дворе шумно. Вы смотрите из окна — что-то "
        "изменилось со вчера, но точно не можете сказать, что именно."
    ),
    "prompt": """\
VERSION: 35.0 | Сценарий: Благоустройство
...
ГЛАВНОЕ ПРАВИЛО — СОХРАНЕНИЕ НЕОПРЕДЕЛЁННОСТИ:
Никогда не подтверждай и не опровергай — идёт ли что-то не так.
Управляющая компания отвечает формально или не отвечает вовсе.
Соседи расходятся во мнениях. Рабочие говорят что всё по плану.
Нет источника который дал бы однозначный ответ.
Ситуация остаётся неясной до конца сессии.
...

{
    ...
    "opening": None,
    "prompt": """\
VERSION: 35.0 | Сценарий: Неопределённая задача: офис
...

{
    ...
    "opening": None,
    "prompt": """\
VERSION: 35.0 | Сценарий: Неопределённая задача: сказка
...
def init_ui():
    """Начальный экран. Консент показывается через gr.Markdown над чатом,
    а не в самом чате — чтобы не сливаться со сценарием."""
    state = {
        "stage":           "awaiting_consent",
        "chat_display":    [],   # чат пустой до согласия
        "off_topic_count": 0,
        "passive_count":   0,
    }
...
def _create_session_log(code: str, scenario_name: str, scenario_idx: int,
    ...
    # Один вызов write_critical для всех метаданных — чтобы Sheets не смещал строки
...
def _bootstrap_session(code, reservation, session_state, user_message,
    ...
    try:
        with open(log_file, "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerows(consent_row)
    except Exception as e:
        logging.error("Failed to log consent: %s", e)
    # Не вызываем write_critical отдельно — consent пишется в очередь вместе
    # с первым ответом модели чтобы Sheets получал данные одним блоком
    enqueue_gsheet(consent_row)
    ...
    # Первый экран: фиксированный opening или только intro
    new_state, disp = _generate_first_turn(new_state, scenario.get("opening"))
    return new_state, disp

def _generate_first_turn(session_state: dict, opening) -> tuple:
    """Формирует первый экран сценария без API-запроса.

    opening — строка: показываем её после разделителя.
    opening — None: участник видит только intro и сразу вводит действие.

    History остаётся пустым — модель начнёт с чистого листа.
    """
    gs:  GameState = session_state["game_state"]
    pid: str       = session_state.get("participant_code", "")

    # Логируем снимок состояния при старте
    snap_row = [
        datetime.now().isoformat(), "SYSTEM",
        f"{LogEvent.STATE_SNAPSHOT}_INTRO: "
        f"{json.dumps(gs.to_log_dict(), ensure_ascii=False)}",
        gs.scenario_name, 0, "", pid,
    ]
    try:
        with open(session_state["log_file"], "a", newline="", encoding="utf-8") as f:
            csv.writer(f).writerow(snap_row)
    except Exception as e:
        logging.error("CSV write error (intro snapshot): %s", e)
    enqueue_gsheet([snap_row])

    # Формируем chat_display
    if opening is not None:
        ...
        # Офис и сказка — только intro, без второго сообщения
        chat_display = session_state["chat_display"]
...
/* Консент-блок — отдельный от чата, с рамкой */
#consent-block {
    background: #ffffff;
...
with gr.Blocks(
    ...
    # ── Стадия 1: консент ────────────────────────────────────────────────────
    with gr.Column(visible=True, elem_id="consent-block") as consent_block:
        consent_panel = gr.Markdown(
            value=INFORMED_CONSENT,
            elem_id="consent-panel",
        )
        consent_input = gr.Textbox(
            label="",
            placeholder="Напишите «Согласен» или «Согласна» и нажмите Enter...",
            show_label=False,
            lines=1,
            max_lines=1,
            autofocus=True,
            container=False,
            elem_id="consent-input",
        )
    ...
    with gr.Row(visible=False, elem_id="input-row") as input_row:
        msg = gr.Textbox(
            label="",
    ...
    uuid_bridge = gr.Textbox(value="", visible=False, elem_id="uuid-bridge")
    session = gr.State()
    ...
    def _handle_consent_input(message, session_state):
        """Обрабатывает ввод на стадии согласия."""
        msg_out, disp, new_state = respond(message, session_state)
        uuid_value = new_state.get("participant_code", "") if new_state else ""
        stage = new_state.get("stage") if new_state else None
        if stage == "playing":
            # Переключаем: скрываем консент, показываем чат
            return (
                "",                          # consent_input очищаем
                gr.update(visible=False),    # consent_block
        ...
        # Согласие не распознано — остаёмся на стадии консента
        return msg_out, gr.update(), gr.update(), gr.update(), disp, new_state, uuid_value
    ...
    # Согласие — отдельный обработчик
    consent_input.submit(
        fn=_handle_consent_input,
        inputs=[consent_input, session],
        outputs=[consent_input, consent_block, chatbot, input_row,
                 chatbot, session, uuid_bridge],
    )
