...
def write_critical(rows: list):
    """Запись критичных событий: пробуем sync, fallback на queue.
    Используется для SESSION_STARTED, GAME_ENDED, CONSENT_GIVEN — чтобы они
    точно дошли до Sheets даже при перезапуске контейнера."""
    if append_to_gsheet_sync(rows):
        return
    # Sync не удался — кладём в очередь как обычно
    enqueue_gsheet(rows)
...
def reserve_or_resume(code: str) -> Optional[dict]:
    ...
    with _codes_lock:
        ...
        events   = random.sample(EVENTS[scenario["name"]], 2)
...
def build_state_block(gs: GameState) -> str:
    ...
    lines = [
        ...
        f"Ход: {gs.turn} / 10",
        f"Ресурс: {gs.resource_label()}",
        f"Событие 1 [{e1}]: {e1_status}",
        f"Событие 2 [{e2}]: {e2_status}",
    ]

    # Сигнал тревоги при критическом ресурсе — модель должна учесть это в логике сценария
    if not gs.is_informant() and gs.resource >= 3:
        lines.append(
            "ВНИМАНИЕ: ресурс на КРИТИЧЕСКОМ уровне. Любое затратное решение "
            "может привести к досрочному завершению сценария — поставь <END_GAME> "
...
EVENTS = {
    ScenarioName.PROJECT: [
        ...
        "Ключевой клиент неожиданно назначил внеплановый ревью-звонок — точная дата пока неизвестна, ориентировочно «в ближайшие дни».",
    ],
    ScenarioName.MISSION: [
        ...
        "База передаёт зашифрованную команду от другого оператора смены — формулировка двусмысленная, неясно, что именно требуется.",
    ],
    ScenarioName.CRISIS: [
        ...
        "Стоит ли открывать гуманитарный коридор через единственный мост — последствия для других районов непредсказуемы.",
    ],
    ...
    ScenarioName.VIRUS: [
        ...
        "Решение о публикации промежуточных данных: реакция научного сообщества и регуляторов непредсказуема.",
        "Регулятор запросил промежуточный отчёт — срок подачи не уточнён: формулировка «в ближайшее время» может означать и три дня, и три часа.",
    ],
}
...

{
    ...
    "intro": (
        "Вы — руководитель важного проекта в крупной компании. "
        "Проект только запущен: у вас есть бюджет, команда из пяти ключевых сотрудников "
        "и высокая репутация компании. "
        "Рядом несколько экспертов-советников, которые уже начали давать рекомендации. "
        "Обстановка динамичная, а доступная информация не всегда точна."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 24.0 | Сценарий: Управление проектом
...
Сигналы участнику: «бюджет под контролем» / «расходы растут» / «ресурсы почти исчерпаны».
...

{
    ...
    "intro": (
        ...
        "Энергия ограничена, территория малоизучена."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 24.0 | Сценарий: Межпланетная миссия
...
Сигналы участнику: «заряда достаточно» / «уровень снижается» / «энергия на исходе».
...

{
    ...
    "intro": (
        "Вы — мэр небольшого города, в котором началось стремительное наводнение. "
        "В вашем распоряжении ограниченный бюджет, спасательные бригады "
        "и неполная информация с мест."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 24.0 | Сценарий: Кризисное управление
...
Сигналы участнику: «ресурсы позволяют действовать» / «бригады перегружены» /
...

{
    ...
    "intro": (
        ...
        "Ваша задача: выяснить, был ли охранник Виктор на посту в момент кражи.\n"
        "Перед вами Виктор — дежурил в ту смену, нервничает, даёт показания. "
        "У вас есть время на допрос, но не бесконечное."
    ),
    "resource_start": 0,
    "prompt": """\
VERSION: 24.0 | Сценарий: Разговор с информантом
...
ТВОЯ СКРЫТАЯ СИТУАЦИЯ:
Ты отлучался с поста ~20 минут — выходил покурить во двор.
Боишься потерять работу. Говоришь, что всё время был на месте.
Не знаешь точное время кражи.

ЛОГИКА СЧЁТЧИКА ДАВЛЕНИЯ:
Текущее значение счётчика давления передаётся в блоке состояния выше.
При значении 3 — оговариваешься: «ну, я на минуту отходил».
При следующем прямом вопросе после оговорки — признаёшься в отлучке.
Прямой вопрос о местонахождении или времени отсутствия = +1 к давлению.
Сообщи в ответе [PRESSURE+1] в скрытой строке после основного текста,
чтобы Python мог обновить счётчик. Участнику эту строку не показывай.
Аналогично: если оговорился — добавь [SLIPPED], если признался — [CONFESSED].
...

{
    ...
    "intro": (
        ...
        "пытающейся создать лечение от новой эпидемии. "
        "Ресурсы ограничены, результаты неоднозначны, "
        "а времени становится всё меньше."
    ),
    "resource_start": 1,
    "prompt": """\
VERSION: 24.0 | Сценарий: Борьба с вирусом
...
Сигналы участнику: «ресурсы позволяют продолжать» / «запасы сокращаются» /
«лаборатория работает на пределе».
...
def _create_session_log(code: str, scenario_name: str, scenario_idx: int,
    ...
    metadata_rows = [
        [ts, "SYSTEM", session_marker, "", "", ""],
        [ts, "SYSTEM", f"PARTICIPANT_ID: {code}", "", "", ""],
        [ts, "SYSTEM", f"SCENARIO: {scenario_name} (index {scenario_idx})", "", "", ""],
        [ts, "SYSTEM", f"EVENT_1: {events[0]}", "", "", ""],
        [ts, "SYSTEM", f"EVENT_2: {events[1]}", "", "", ""],
        [ts, "SYSTEM", f"PROMPT_VERSION: {PROMPT_VERSION}", "", "", ""],
        [ts, "SYSTEM", f"TEMPERATURE: {TEMPERATURE}", "", "", ""],
    ]
    try:
        with open(log_file, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["timestamp", "role", "content", "scenario", "turn", "time_seconds"])
            w.writerows(metadata_rows)
    except Exception as e:
        logging.error("Failed to create log file: %s", e)

    # Синхронная запись метаданных в Sheets — гарантия фиксации старта сессии
...
def respond(message: str, session_state: dict):
    ...
    if session_state is None:
        session_state, _ = init_ui()

    if not message or not message.strip():
        return "", session_state["chat_display"], session_state
...
def _process_turn(message: str, session_state: dict):
    ...
    # успешного ответа, чтобы не плодить записи при сбоях API
    ...
    # Срабатывание событий — логируем факт с текстом события
    triggered_log: list = []
    maybe_trigger_event(gs, log_rows=triggered_log)
    # Флаги уже установлены в gs_pending (он стал gs). Дублируем только лог.
    ...
    if game_ended:
        ...
        end_row = [[datetime.now().isoformat(), "SYSTEM",
                    ...
                    gs.scenario_name, gs.turn, ""]]
        log_rows.extend(end_row)
        # Критичное событие — синхронная запись в Sheets
        write_critical(end_row)
        bot_reply_display += (
            "\n\n---\n"
